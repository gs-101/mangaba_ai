...
class SlackChannelAnalyzer:
    ...
    async def analyze_channel(
        ...
        try:
            ...
            analysis.summary = (
                f"Análise do canal #{channel_name}:\n\n{analysis.summary}"
            )
            ...
            if e.response["error"] == "channel_not_found":
                raise Exception(
                    f"Canal não encontrado. Verifique se o ID está correto e se o bot tem acesso ao canal."
                )
            elif e.response["error"] == "not_in_channel":
                raise Exception(
                    f"O bot não está no canal. Convide o bot para o canal primeiro."
                )
            else:
