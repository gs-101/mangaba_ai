...
class SlackIntegration:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        self.socket_client = SocketModeClient(
            app_token=os.getenv("SLACK_APP_TOKEN"), web_client=self.slack_client
        )
    ...
    async def handle_slack_message(
        ...
        try:
            ...
            await self.mcp_protocol.fuse_context(
                prompt=f"Mensagem do Slack: {text}", context=context
            )
            ...
            await self.mcp_protocol.fuse_context(
                prompt=f"Análise da conversa: {analysis}", context=context
            )
