...
class SlackAgentHandler:
    def __init__(self, bot_token: str = None):
        ...
        if not self.bot_token:
            raise ValueError(
                "Bot token is required. Please provide it or set SLACK_BOT_TOKEN environment variable."
            )
    ...
    def _handle_analysis_request(
        self, channel_id: str, message: str, thread_ts: str, user_id: str
    ) -> None:
    ...
    def _analyze_messages(self, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Sort user activity
        analysis["user_activity"] = dict(
            sorted(analysis["user_activity"].items(), key=lambda x: x[1], reverse=True)
        )
    ...
    def _handle_summary_request(self, channel_id: str, thread_ts: str) -> None:
        ...
        try:
            ...
            result = self.client.conversations_history(
                channel=channel_id, limit=50  # Last 50 messages
            )
