...
from .core.models import Agent, GeminiModel, Task
...
class MangabaAI:
        ...
        try:
            ...
            if not config_file.exists():
                raise ConfigurationError(
                    f"Arquivo de configuração não encontrado: {config}"
                )
            ...
            raise ConfigurationError(
                f"Erro ao decodificar arquivo de configuração: {str(e)}"
            )
        except Exception as e:
        ...
        return {
                ...
                "gemini": {
                    ...
                    "generation_config": {
                        "max_output_tokens": int(
                            os.getenv("GEMINI_MAX_TOKENS", "2048")
                        ),
                        "candidate_count": 1,
                        "stop_sequences": [],
                    },
            ...
            "agents": {
                "max_concurrent_tasks": int(
                    os.getenv("MANGABA_MAX_CONCURRENT_TASKS", "5")
                ),
            ...
            "memory": {
                ...
                "cleanup_interval": int(
                    os.getenv("MANGABA_MEMORY_CLEANUP_INTERVAL", "300")
                ),
    ...
    async def broadcast_message(
        self, sender: str, content: str, roles: Optional[List[str]] = None
    ) -> None:
        """
    ...
    def list_agents(self) -> List[Dict[str, str]]:
        ...
        """
        return [
            {"name": name, "role": agent.role, "goal": agent.goal}
...
async def main():
    ...
    try:
        ...
        researcher = mangaba.create_agent(
            name="pesquisador", role="Pesquisador", goal="Realizar pesquisas"
        )

        analyst = mangaba.create_agent(
            name="analista", role="Analista", goal="Analisar dados"
        )

        writer = mangaba.create_agent(
            name="escritor", role="Escritor", goal="Escrever resumos"
        )

        # Cria tarefas
        task1 = mangaba.create_task(
            description="Pesquisar sobre IA generativa", agent=researcher
        )

        task2 = mangaba.create_task(
            description="Analisar os resultados da pesquisa", agent=analyst
        )

        task3 = mangaba.create_task(
            description="Escrever um resumo dos resultados", agent=writer
        )
