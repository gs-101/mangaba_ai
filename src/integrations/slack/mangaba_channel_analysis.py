...
def format_analysis_results(analysis):
    ...
    for user, count in sorted(
        analysis["user_activity"].items(), key=lambda x: x[1], reverse=True
    )[:5]:
...
def analyze_mangaba_channel(output_type="terminal"):
    ...
    try:
        ...
        if not bot_token or not channel_id:
            raise ValueError(
                "Missing required environment variables: SLACK_BOT_TOKEN or SLACK_CHANNEL_ID"
            )

        analyzer = MangabaChannelAnalyzer(
            bot_token=bot_token, workspace_id="T013NTSNZSP", channel_id=channel_id
        )

        # Register specialized agents for Mangaba analysis
        analyzer.handler.register_agent(
            "repo_analyzer",
            {
        ...
        try:
            ...
            if not result["ok"]:
                raise Exception(
                    f"Failed to get channel messages: {result.get('error')}"
                )
            ...
            for msg in messages:
                ...
                if user:
                    analysis["user_activity"][user] = (
                        analysis["user_activity"].get(user, 0) + 1
                    )
                ...
                # Categorize topics
                if any(
                    word in text for word in ["code", "function", "class", "method"]
                ):
                    analysis["topics"]["code"] += 1
                elif any(
                    word in text for word in ["architecture", "design", "structure"]
                ):
            ...
            if analysis["time_distribution"]:
                most_active_hour = max(
                    analysis["time_distribution"].items(), key=lambda x: x[1]
                )[0]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze Slack channel and display results"
    )
    parser.add_argument(
        "--output",
        choices=["terminal", "slack", "file", "all"],
        default="terminal",
        help="Where to display results",
    )
