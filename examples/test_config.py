...
def check_config_file() -> Dict[str, Any]:
    ...
    required_configs = {
        "api_keys": {
            "gemini": {"required": True, "description": "Chave da API do Google Gemini"}
        },
        "models": {
            "gemini": {
                "required": True,
    ...
    results = {"config_file_exists": False, "configs": {}, "all_valid": False}
        ...
        for section, requirements in required_configs.items():
            ...
            for field, field_req in requirements.items():
                if field_req.get("required", False) and field not in config[section]:
                    logger.error(
                        f"Campo obrigatório '{field}' não encontrado em '{section}'"
                    )
                ...
                if field in config[section]:
                    ...
                    if "fields" in field_req:
                        ...
                        for subfield, subreq in field_req["fields"].items():
                            if (
                                subreq.get("required", False)
                                and subfield not in config[section][field]
                            ):
                                logger.error(
                                    f"Campo obrigatório '{subfield}' não encontrado em '{section}.{field}'"
                                )
                                field_valid = False
                                continue
                            ...
                            if subfield in config[section][field]:
                                try:
                                    ...
                                    if not isinstance(value, expected_type):
                                        logger.error(
                                            f"Tipo inválido para '{section}.{field}.{subfield}': esperado {expected_type.__name__}"
                                        )
                                        field_valid = False
                                except Exception as e:
                                    logger.error(
                                        f"Erro ao validar '{section}.{field}.{subfield}': {str(e)}"
                                    )
                                    field_valid = False
...
async def test_model_connection():
    ...
    try:
        ...
        agent = mangaba.create_agent(
            name="test_agent", role="tester", goal="Testar a conexão com o modelo"
        )

        # Cria uma tarefa simples
        task = mangaba.create_task(
            description="Responda com uma mensagem simples de teste", agent=agent
        )
